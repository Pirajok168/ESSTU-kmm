package ru.esstu.features.news.announcement.domain
...
fun DataResponse.toAnnouncements(): List<NewsNode> {
    ...
    return conversations.filter { it.type == "ANNOUNCEMENT" }.mapNotNull { conversation ->
        val creator = loadedUsers.firstOrNull { it.id == conversation.creatorId }?.toUser()
            ?: return@mapNotNull null
        val messageId =
            dialogs.firstOrNull { it.peerId == conversation.id.toString() }?.lastMessageId
