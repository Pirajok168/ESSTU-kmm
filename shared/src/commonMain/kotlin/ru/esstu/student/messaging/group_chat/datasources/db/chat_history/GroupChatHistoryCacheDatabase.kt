package ru.esstu.student.messaging.group_chat.datasources.db.chat_history
...
import ru.esstu.student.messaging.group_chat.datasources.db.chat_history.entities.MessageGroupChatWithRelatedEntity
...
class GroupChatHistoryCacheDatabase(
    ...
    override suspend fun insertMessage(messages: GroupChatMessage) {
        messages.apply {
            dbQuery.insertMessage(appUserId, idGroupChatMessage, conversationId, fromGroup, replyMessageId, date, message, status)
        }
    }
    ...
    override suspend fun insertAttachments(attachments: List<GroupChatAttachment>) {
        attachments.forEach {
            it.apply {
                dbQuery.insertAttachments(idAttachment, messageIdAttachment, fileUri, LocalFileUri, name, ext, size, type)
            }
        }
    }
    ...
    override suspend fun insertReply(reply: GroupChatReplyMessage) {
        reply.apply {
            dbQuery.insertReply(idReplyMessage, fromSendReplyMessage, dateReplyMessage, messageReplayMessage, attachmentsCount, messageIdReply)
        }
    }
